...
const CBCSForm = () => {
  ...
  // Fetch Staffs filtered by department
  useEffect(() => {
  ...
  const handleDeptSelect = (dept) => {
    setFormData(prev => ({ ...prev, department: dept.dept_name, departmentId: dept.dept_id }));
  ...
  const handleLoadSubjects = async (subjectId) => {
    ...
    try {
      ...
      setSubjectInputs(prev => prev.map(s => 
        s.id === subjectId ? { ...s, subjects: data } : s
      ));
    } catch (err) {
      setError(err.message);
    }
  ...
  const addSubjectField = () => setSubjectInputs(prev => 
    [...prev, { id: Date.now(), subject_id: "", subject_name: "", subjects: [], staffs: [] }]
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      // ⚠️ New fields (backend will fill these after Excel generation)
      allocation_excel_path: "",     // backend will generate
      student_list_excel_path: "",   // backend will generate
      cbcs_link: "", 
      //total_students:0,      
      //submissions:[],  // backend will generate using cbcs_id
      //complete: false                // default, backend will keep it false
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 py-12 px-4">
      ...
      <motion.div className="max-w-3xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">CBCS Course Management</h1>
          <p className="text-gray-600">Select batch, department, semester, subjects, and teaching staff</p>
        </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <input type="text" name="batchYear" placeholder="Batch Year e.g. 2023" value={formData.batchYear} onChange={handleInputChange} className="px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500 w-full" required />
                    <div className="relative">
                      <button type="button" onClick={() => setShowDeptDropdown(!showDeptDropdown)} className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500">
                        {formData.department || "Select Department"}
                        {showDeptDropdown ? <FiChevronUp /> : <FiChevronDown />}
                      </button>
                      <AnimatePresence>
                        {showDeptDropdown && (
                          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            {isLoadingDept ? (
                              <div className="px-4 py-3 text-gray-500">Loading...</div>
                            ) : (
                              departments.map(dept => (
                                <button key={dept._id} type="button" className="w-full text-left px-4 py-3 hover:bg-purple-50" onClick={() => handleDeptSelect(dept)}>
                                  {dept.dept_name}
                                </button>
                              ))
                            )}
                    ...
                    <select name="semester" value={formData.semester} onChange={handleInputChange} className="px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500" required>
                  ...
                  <div className="mt-6">
                    ...
                    <input type="file" accept=".xlsx,.xls" onChange={handleExcelUpload} className="block w-full text-gray-700 border border-gray-300 rounded-lg cursor-pointer p-2"/>
                    {totalStudents > 0 && (
                ...
                <div className="mb-10">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-semibold text-gray-800 flex items-center"><FiBookOpen className="mr-2 text-purple-500" />Subjects & Staff</h2>
                    <button type="button" onClick={addSubjectField} className="flex items-center px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600"><FiPlus className="mr-2"/>Add Subject</button>
                  </div>
                  <div className="space-y-6">
                    <AnimatePresence>
                      {subjectInputs.map((sub, idx) => {
                        ...
                        return (
                          <motion.div key={sub.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="p-6 bg-gray-50 rounded-xl border border-gray-200">
                            <div className="flex justify-between items-center mb-4">
                              <h3 className="font-medium text-gray-700">Subject {idx + 1}</h3>
                              {subjectInputs.length > 1 && (
                                <button type="button" onClick={() => removeSubjectField(sub.id)} className="text-red-500 hover:text-red-700"><FiTrash2 /></button>
                              )}
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                              ...
                              <button type="button" onClick={() => handleLoadSubjects(sub.id)} className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">Load Subjects</button>
                            </div>
                            ...
                            <div className="mt-4">
                              <div className="flex justify-between items-center mb-3">
                                ...
                                <button type="button" onClick={() => addStaffToSubject(sub.id)} className="flex items-center px-3 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600"><FiUserPlus className="mr-1" />Add Staff</button>
                              </div>
                              <div className="space-y-3">
                                <AnimatePresence>
                                  {sub.staffs.map((stf,index) => (
                                    <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex items-center space-x-3">
                                      <select value={stf.staff_id} onChange={e => handleStaffChange(sub.id, index, e.target.value)} className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500">
                                        <option value="">Select Staff</option>
                                        {staffsList.map(st => <option key={st.staff_id} value={st.staff_id}>{st.name}</option>)}
                                      </select>
                                      ...
                                      <button type="button" onClick={() => removeStaffFromSubject(sub.id, index)} className="text-red-500 hover:text-red-700"><FiTrash2 /></button>
                                    </motion.div>
                                  ))}
                                </AnimatePresence>
                                {totalStudents > 0 && (
                ...
                <button type="submit" className="w-full py-4 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-lg font-semibold hover:from-purple-600 hover:to-blue-600">Submit Form</button>
              </form>
            </div>
          </div>
        </motion.div>
